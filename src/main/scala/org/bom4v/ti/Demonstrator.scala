...
val cdr_data_t = spark.read.option("header","true").option("inferSchema","true").option("delimiter", "^").csv("cdr_example.csv")

val cdr_data_temp = cdr_data_t.selectExpr("specificationVersionNumber",
...
val cdr_data = cdr_data_temp.withColumn("callEventStartDay",cdr_data_temp("callEventStartTimeStamp").cast(DateType))
...
val locationAreasTech = cdr_data.groupBy("imsi","locationArea").pivot("locationArea").agg(mean("callEventDuration"), min("callEventDuration"), 
			max("callEventDuration"), sum("callEventDuration"))
val cellIdsTech = cdr_data.groupBy("imsi","cellId").pivot("cellId").agg(mean("callEventDuration"), min("callEventDuration"), 
			max("callEventDuration"), sum("callEventDuration"))
